...
/* Dummy byte offset for fast, dual and quad
 * reads
 */
#define DUMMY_SIZE 1
/* Number of dummy bytes for fast, dual and
 * quad reads
 */
...
#define TEST_ADDRESS 0xF0000000
...
int QspiFlashPolledExample(XQspiPs * QspiInstancePtr, u16 QspiDeviceId, int operations);
 ...
 *
 ******************************************************************************/
int MacIPSetup(int operations) {
  int Status;

  //xil_printf("QSPI FLASH Polled Example Test \r\n");

  /* Run the Qspi Interrupt example.*/
  Status = QspiFlashPolledExample( & QspiInstance, QSPI_DEVICE_ID, operations);
...
int QspiFlashPolledExample(XQspiPs * QspiInstancePtr, u16 QspiDeviceId, int operations) {
  ...
  for(j=0;j<6;j++) WriteBuffer[DATA_OFFSET+j] = (u8)(MAC[j]);
  for(j=0;j<4;j++) WriteBuffer[DATA_OFFSET+j+6] = (u8)(IP[j]);
  for(j=0;j<4;j++) WriteBuffer[DATA_OFFSET+j+10] = (u8)(MSK[j]);
  for(j=0;j<4;j++) WriteBuffer[DATA_OFFSET+j+14] = (u8)(GW[j]);
  ...
  if (operations == 1) {
    ...
    //for (i = 0; i < 30; i++) xil_printf("%d ", WriteBuffer[i]);
  ...
  for(j=0;j<6;j++) MAC[j]= ReadBuffer[4+j];
  for(j=0;j<4;j++) IP[j]= ReadBuffer[10+j];
  for(j=0;j<4;j++) MSK[j]= ReadBuffer[14+j];
  for(j=0;j<4;j++) GW[j]= ReadBuffer[18+j];
